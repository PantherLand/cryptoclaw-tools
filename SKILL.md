...
- `POLYMARKET_API_URL` is optional unless you need a non-default Polymarket bridge endpoint
...
- Supports EVM source chains only: `ethereum`, `polygon`, `arbitrum`, `base`
- Supports ERC-20 deposits only in this first version
- Uses Polymarket's Bridge API for supported-asset discovery, deposit address generation, quoting, and status lookup
- Interprets `destination` as the Polymarket wallet that should be credited
- Transfers funds to a Polymarket-generated deposit address and returns that address explicitly
- Runs treasury safety checks before execution
- Returns quote details including `estimatedReceivedUsdcE`, `minimumReceivedUsdcE`, and `statusEndpoint`

Recommended agent workflow:

- Call `quote_operation` first
- Review `depositAddress`, `estimatedReceivedUsdcE`, `minimumReceivedUsdcE`, and `statusEndpoint`
- Call `deposit_to_polymarket` with `dryRun=true`
- Only then call `deposit_to_polymarket` with `dryRun=false`
...
```bash
node dist/index.js --action deposit_to_polymarket --input '{"sourceChain":"base","token":"USDC","amount":"3","destination":"0xYourPolymarketWallet","approval":true,"dryRun":true}'
```
...
```bash
node dist/index.js --action quote_operation --input '{"operationType":"deposit_to_polymarket","sourceChain":"base","token":"USDC","amount":"3","destination":"0xYourPolymarketWallet"}'
```
