...
export class HyperliquidTradingService {
  ...
  public async quoteWithdraw(input: {
    destination: Address;
    amount?: string;
  }): Promise<Record<string, unknown>> {
    this.assertTradingEnabled();
    const user = this.resolveUserAddress();
    const accountState = await this.infoClient.clearinghouseState({ user });
    const withdrawable = toNumber(accountState.withdrawable);
    const feeUsd = 1;
    const requestedAmount = input.amount ? Number(input.amount) : Math.max(withdrawable - feeUsd, 0);

    if (!Number.isFinite(requestedAmount) || requestedAmount <= 0) {
      throw new Error("No withdrawable USDC is available after the Hyperliquid withdrawal fee.");
    }

    if (requestedAmount + feeUsd > withdrawable + 1e-9) {
      throw new Error(
        `Requested Hyperliquid withdrawal ${formatNumber(requestedAmount, 6)} USDC plus fee ${feeUsd} exceeds withdrawable balance ${formatNumber(withdrawable, 6)} USDC.`
      );
    }
    ...
    return {
      ...
      withdrawable: formatNumber(withdrawable, 6),
      requestedAmount: formatNumber(requestedAmount, 6),
      fixedFeeUsd: formatNumber(feeUsd, 2),
      expectedWalletReceive: formatNumber(requestedAmount, 6),
      expectedRemainingOnHyperliquid: formatNumber(Math.max(withdrawable - requestedAmount - feeUsd, 0), 6)
    };
  }
  ...
  public async withdraw(input: {
    ...
    const response = await exchangeClient.withdraw3({
      destination: input.destination,
      amount: quote.requestedAmount as string
    });
