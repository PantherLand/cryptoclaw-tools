...
export async function quoteOperationTool(
  ...
  if (parsed.operationType === "deposit_to_polymarket") {
    if (!parsed.sourceChain || !parsed.token || !parsed.amount || !parsed.destination) {
      return buildFailure("quote_operation", requestId, "error", [
        "deposit_to_polymarket quote requires sourceChain, token, amount, and destination."
      ]);
    }
    if (parsed.sourceChain === "solana") {
      return buildFailure("quote_operation", requestId, "error", [
        "deposit_to_polymarket does not support Solana source chains yet."
      ]);
    }

    const quote = await polymarketService.quoteDeposit({
      sourceChain: parsed.sourceChain,
      token: parsed.token,
      amount: parsed.amount,
      destination: parsed.destination
  ...
  return buildFailure("quote_operation", requestId, "error", [
    "quote_operation currently supports transfer_token, swap_token, bridge_token, deposit_to_hyperliquid, deposit_to_polymarket, place_hyperliquid_order, and protect_hyperliquid_position."
  ]);
}
