...
The first Polymarket deposit implementation is intentionally conservative:

- It supports EVM source chains only: Ethereum, Polygon, Arbitrum, and Base
- It supports ERC-20 deposits only
- The `destination` input is the Polymarket wallet that should be credited
- The actual onchain recipient is a Polymarket-generated deposit address returned by the tool
- Quotes, minimums, and status come from Polymarket's official Bridge API
- Settlement is asynchronous, so execution responses include a `statusEndpoint` for follow-up checks
...
7. `POLYMARKET_API_URL` is optional unless you need a non-default Polymarket bridge endpoint.
...
```bash
node dist/index.js --action deposit_to_polymarket --input '{
  "sourceChain": "base",
  "token": "USDC",
  "amount": "3",
  "destination": "0xYourPolymarketWallet",
  "approval": true,
  "dryRun": true
}'
```
...
1. Run `quote_operation` with `operationType: "deposit_to_polymarket"`.
2. Review `depositAddress`, `estimatedReceivedUsdcE`, `minimumReceivedUsdcE`, and `statusEndpoint`.
3. Run `deposit_to_polymarket` with `dryRun: true`.
4. Only then run `deposit_to_polymarket` with `dryRun: false`.
